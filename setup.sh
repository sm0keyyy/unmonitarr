#!/bin/bash
# Setup script for Unmonitarr container environment

# Create directory structure
mkdir -p config logs

# Create config file if it doesn't exist
if [ ! -f "config/unmonitarr_config.json" ]; then
    cat > config/unmonitarr_config.json << 'EOF'
{
  "general": {
    "release_groups": [
      "yify",
...
EOF
    echo "Created default config file at config/unmonitarr_config.json"
    echo "⚠️ Please edit this file to add your API keys before running the container"
else
    echo "Config file already exists at config/unmonitarr_config.json"
fi
...
# Download the Python script from the artifact to the current directory
echo "Extracting unmonitarr.py script..."
cat > unmonitarr.py << 'EOF'
#!/usr/bin/env python3
"""
Unmonitarr - Universal Unmonitor Script for Radarr/Sonarr
